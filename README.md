...
- **macOS on Apple Silicon (M1/M2/M3/M4).** This build is arm64-only and
  will not open on Intel Macs ("can't be opened" error). Detection runs
  GPU-accelerated via Metal (MPS) automatically — no configuration needed.
...
make_dmg.sh               Packages CMAD.app into a shareable CMAD-Installer-arm64.dmg
...
./make_dmg.sh           # produces CMAD-Installer-arm64.dmg
```

This must be run **on an actual Apple Silicon Mac** — PyInstaller bundles
whatever's locally installed, including torch's Metal backend, which only
exists on real Apple hardware. The resulting app is arm64-only and will
...
1. **Requires an Apple Silicon Mac** (M1/M2/M3/M4). Check via Apple menu
   (top-left) → About This Mac — under "Chip," it should say "Apple
   M1/M2/M3/M4." This build will not open on an Intel Mac.
2. Download `CMAD-Installer-arm64.dmg` — you can download it directly from
   here:
   [CMAD macOS Installer (Google Drive)](https://drive.google.com/drive/folders/1KsU4wJbA3mRALOhNnesAkyFwNFKVzssQ?usp=sharing)
3. Open it, drag `CMAD.app` to Applications.
4. First launch: right-click `CMAD.app` → Open → Open (one-time, since
   the app is unsigned).
5. Enter two dates and run a detection.

## Known limitations

- Requires internet at runtime (NOAA image downloads).
- **Apple Silicon only** — will not open on Intel Macs.
...
© iHARP [NSF HDR Institute](https://iharp.umbc.edu/)
